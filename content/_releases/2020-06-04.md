...
Introducing a new centralised service (code-named **Faceless**) to handle the
secrets and OAuth credentials. We aim to replace the credential handling mechanism
in the whole platform. **The service is still in early implementation stage.**
...
3.  Component MUST ask platform for a valid value of secret every time it needs it. Caching will not not be recommended.
4.  Use of `this.emit('updateKeys')` mechanism would be discouraged. It will still works, and continue to exist until the `Credentials` mechanism is deprecated.
5.  `Secrets` and `Credentials` must not be used simultaneously.

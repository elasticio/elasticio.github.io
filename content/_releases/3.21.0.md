...
Another page for Recipes UI is Recipe activation [page](app.{{site.data.tenant.name}}/c/:contract_id#/w/:workspace_id/recipes/:recipe_id/activate). It contains Recipe variables fields and Component sections listed with Credentials. Here you can choose existing Credentials, create new or edit existing ones during Recipe activation. In case the data for Component configure input section fields is not defined in the Recipe, you can fill the fields with custom data.


## Quota
You can now see monthly quota history of a Contract on the corresponding page:

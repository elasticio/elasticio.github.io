...
As a part of the platform optimisation and extending of its capability to withstand a high load.
...
The new page `Containers` that will display information related to containers and components inside. 
The main page displays logs for "Today" by default, and users are available to extend the date range by the filtering and Search fileds.  The container logs are saving up to 30 days.
The `Containers` page displays next States:
* **Running** *(green flag)* - The container is currently running
* **Finished** *(green flag)* - The container is successfully shutted down 
...
Now, the Lookout not process data/input messages to reduce the mongodb load.
We introduced the new env var - `LOOKOUT_PROCESS_DATA` (default: `false`).
It controls whether lookout consume `data` and `input` non-retry messages.

### New data source for dashboard

As we disabled the data messages processing in lookout we created a new data source for Dashboard.
...
We added query parameter `container_id` into `GET v2/logs` [endpoint](
...
Before deploying Tenant-Admin should update the feature flag for the necessary contracts with `disableRecipeFeature: false` to be able to continue to use the Recipe feature.
```
{
    "data": {
        "type": "contract",
...
The special solution to fix an issue in CSS styles.
With the next release, this section will be transfered to React.

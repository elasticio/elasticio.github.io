...
* Transferred Workspace Memory Usage graph

### Transfer modal windows to the new styles:
* Transform UI for Flow Designer: Configuration for Content-Based Router Component to React - Now users can map values from previous steps for evaluation.
...
We've removed this env var from the frontend completely, and removed it from a lookout by making it run as if this env var is set to "false".

### Change elasticsearch index config:

We've improved the Logs search to better handle partial text matching (e.g. "test.123" log line previously would match only if a user searched for exact "test.123", but now both "test" and "123" will match the logline).

Also, we've fixed the bug when irrelevant search results appeared due to incorrect configuration of Elasticsearch.

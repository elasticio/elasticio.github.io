...
You can now see all your logs on the dedicated Logs Page. While the most important logs are seen on the [Executions](/getting-started/executions.html) Page, we decided no to spam you with everything else, and created a special page for all the logs. The new page lists all the logs for all the Flows in the given workspace. You can filter the list by:
- Flow name:
...
The possible levels: 1 - None (log level None means default log level when it’s not passed), 10 - trace, 20 - debug, 30 - info, 40 - warn, 50 - error, 60 - fatal.

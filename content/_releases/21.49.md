...
In order to speed up the work of the flow draft page, we decided to improve a caching system. We added `if-none-match`(https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/If-None-Match) by default header to the API proxy mechanism to improve responses caching on the client-side.

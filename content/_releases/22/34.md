...
### Meaningful error for invalid SSH key entry
...
types. Now the user-interface shows a distinct error explaining the situation if
the user enters an un-supported or malformed SSH Key instead of just disabling
the Save button like it was before.
...
Our team has just begun working on a new exciting feature to enable deployment of
externally build docker images for the integration components. This feature might
replace or provide an alternative to git push deployment of user developed components.
...
*   `docker_registry` object into the an HTTP `POST` call to `/v2/teams/`
...
The flow execution statistics presented on the Dashboard Runlog is stored
in the platform Mongo Database. This is in contrast to the flow execution threads, shown
on the Executions pages, which is stored separately in the Clickhouse.
...
which you can use to set retention suitable for your environment. If not specified,
the **default value is set 432000 seconds (5 days)**. Which means the platform will store
the Dashboard Runlog record only for 5 days in the Database.

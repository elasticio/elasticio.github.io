...
*   Set a new parameter called `USER_2FA_CRYPTO_PASSWORD` in the platform secrets part of the HELM3 charts. You must generate it in advance on Google's side. **If this value is not set all the following steps will fail.**
...
*   When user enabled the 2FA and fails to login for some reson you can `POST` to a new API endpoint `/v2/users/:id/disable-totp` to clear 2FA setup and enable user to login again using only username and password. 

...
With platform {{page.version}} we added a UI support for deleting pending invitations
to contracts. This functionality existed before only as an
...
*   Set a new parameter called `USER_2FA_CRYPTO_PASSWORD` in the platform secrets part of the HELM3 charts. You must generate it in advance. **If this value is not set all the following steps will fail.**
*   Add a new tenant level parameter (switch) `attributes.feature_flags.enabled_totp` and set it `true` using a `PATCH` to `/v2/tenants/{TENANT_ID}` record (admin access).
*   When user enabled the 2FA and fails to login for some reason you can `POST` to a new API endpoint `/v2/users/:id/disable-totp` to clear 2FA setup and enable user to login again using only username and password.


### Support HTTPS for docker registry URI

With this release we added HTTPS for external docker resgistry URI.
...
add IDs of all contracts to the components records. All current contracts will
have access to all available components. Any subsequent new contracts would only have
access to all components in the developer workspaces.

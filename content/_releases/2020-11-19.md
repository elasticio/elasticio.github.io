...
> defined by the law. You can, however, add an additional roles to the workspace
> `Owner` which has the permission to create recipes.

In addition, the Tenant Administrator role can expert a flow to a recipe if he/she
has access to the flow. These changes are also applied in the
[API documentation]({{site.data.tenant.apiBaseUri}}/docs/v2/#create-a-recipe).

To benefit from these changes a migration process must be done which is handeled
by the gendry.

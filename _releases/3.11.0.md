...
- [Get Tenant’s roles]({{tenant.apiBaseUri}}/docs/v2/#get-tenant's-roles)
- [Update Tenant’s roles]({{tenant.apiBaseUri}}/docs/v2/#update-tenant's-roles)
- [Get the list of available permissions]({{ api_base_url }}/docs/v2/#get-the-list-of-available-permissions)
...
#### The endpoint to get roles per contract is introduced. 
Endpoint [Get the Contract’s roles]({{tenant.apiBaseUri}}/docs/v2/#get-the-contract's-roles) returns all the roles that are that are assigned to the corresponding contract.
...
- [Get the list of available permissions]({{tenant.apiBaseUri}}/docs/v2/#get-the-list-of-available-permissions)
- [Get Tenant’s roles]({{tenant.apiBaseUri}}/docs/v2/#get-tenant's-roles) and [Update Tenant’s roles]({{tenant.apiBaseUri}}/docs/v2/#update-tenant's-roles)
- [Get the Contract’s roles]({{tenant.apiBaseUri}}/docs/v2/#get-the-contract's-roles)
...
Frontend retrieves an available roles' list from the [Get the Contract’s roles]({{tenant.apiBaseUri}}/docs/v2/#get-the-contract's-roles) endpoint, instead of using a hardcoded list.
...
Introduced new attribute `available_roles` in the next endpoints:
- [Get Contract by Id]({{tenant.apiBaseUri}}/docs/v2/#get-contract-by-id) allow to retrieve `available_roles`
- [Create a Contract]({{tenant.apiBaseUri}}/docs/v2/#create-a-contract) allow to assign `available_roles`
- [Update a Contract]({{tenant.apiBaseUri}}/docs/v2/#update-a-contract) allow to update `available_roles`
...
This request authorize only for `tenant-admin`
...
Endpoints [Verify credentials]({{tenant.apiBaseUri}}/docs/v2/#verify-credentials), [Retrieve component’s metamodel]({{tenant.apiBaseUri}}/docs/v2/#retrieve-component's-metamodel) and [Retrieve component’s select model]({{tenant.apiBaseUri}}/docs/v2/#retrieve-component's-select-model) return `400 Bad Request` in case of absent relation with workspace. 

#### It is possible to suspend a contract:
Suspending a contract possibility. The following endpoints are implemented:
- [Suspend Contract]({{tenant.apiBaseUri}}/docs/v2/#suspend-contract)
- [Unsuspend Contract]({{tenant.apiBaseUri}}/docs/v2/#unsuspend-contract)
...
- [Suspend Contract]({{tenant.apiBaseUri}}/docs/v2/#suspend-contract). Mark contract as suspended, send stop to all flows, but don't wait. Wiper will wait). Allowed only for `active` contracts.
- [Unsuspend Contract]({{tenant.apiBaseUri}}/docs/v2/#unsuspend-contract). Allowed only for `suspended` contracts.
- The new attribute `status` is added to the next endpoints: [Get Contracts]({{ api_base_url }}/docs/v2/#get-contracts) and [Get Contract by Id]({{tenant.apiBaseUri}}/docs/v2/#get-contract-by-id). May have three values `active`, `suspended`, `suspending`.

##### Auth

* New endpoints are allowed to use only by `SystemAccounts`
* Old/Outdated endpoints ([Get Contracts]({{tenant.apiBaseUri}}/docs/v2/#get-contracts) and [Get Contract by Id]({{tenant.apiBaseUri}}/docs/v2/#get-contract-by-id)) work with the same authorization rules, as always. A new attribute was added. It is always visible.
...
We show notice/warning message *"This contract has been suspended. Please, contact the support team for more information."* on the contact page, to make it easy to understand that a contract is suspended. All the buttons that allow creating/editing any entities were hidden.
